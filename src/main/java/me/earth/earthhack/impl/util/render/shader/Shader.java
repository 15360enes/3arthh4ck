...
public abstract class Shader implements Nameable
{
    ...
    public Shader(String name, String path)
    {
        ...
        {
            Duny4hil3.getLogger().error("Shader " + name + " failed to read.");
            e.printStackTrace();
            return;
        }
        ...
        {
            Duny4hil3.getLogger().error("Shader " + name + "'s vertex shader failed to compile! Reason: " + glGetShaderInfoLog(vertex, 1024));
            return;
        }

        if (glGetShaderi(vertex, GL_COMPILE_STATUS) == GL11.GL_FALSE)
        {
            Duny4hil3.getLogger().error("Shader " + name + "'s fragment shader failed to compile! Reason: " + glGetShaderInfoLog(fragment, 1024));
            return;
        }
        ...
        {
            Duny4hil3.getLogger().error("Shader " + name + "failed to link! Reason: " + glGetProgramInfoLog(fragment, 1024));
            return;
        }
        ...
        {
            Duny4hil3.getLogger().error("Shader " + name + "failed to validate! Reason: " + glGetProgramInfoLog(fragment, 1024));
            return;
        }
